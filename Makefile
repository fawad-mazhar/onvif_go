...
# Build WSD server
wsd-server:
	go build $(LDFLAGS) -o $(BUILD_DIR)/$(WSD_BINARY_NAME) $(MAIN_DIR)/server/wsd_server.go

# Build notification server
notify-server:
	go build $(LDFLAGS) -o $(BUILD_DIR)/$(NOTIFY_BINARY_NAME) $(MAIN_DIR)/server/notification_server.go
